...
function init() {
    ...
    //     }
    // }
    var mesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(3264*0.3,2448*0.3),
                              new THREE.MeshBasicMaterial({
                                map: THREE.ImageUtils.loadTexture("../tex/IMG_2560.JPG")
                              }));
    ...
    effectHBlur.uniforms[ 'h' ].value = 1.0 / ( window.innerWidth /2 );
    effectVBlur.uniforms[ 'v' ].value = 1.0 / ( window.innerHeight /2 );
...
function fbInit(){
    ...
    fbShaders = [passShader, passShader, diffShader, colorShader, blurShader, warpShader];
...
function fbDraw(){
    for(var i = 0; i < fbMaterial.fbos.length; i++){
      fbMaterial.fbos[i].material.uniforms.mouse.value = new THREE.Vector2(Math.sin(time)*0.25, Math.cos(time)*0.25);
    }
